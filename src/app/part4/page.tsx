...
function Part4Trainer() {
  ...
  const sessionKey = `toeic_p4sess_${testId}`;
  type SavedSession = {
  ...
  const handleNextSet = () => {
    if (currentSetIndex + 1 < sets.length) {
      ...
      completeActiveTaskByType('practice', { part: 'p4' });
      ...
      <div className={styles.container}>
        ...
        <div className={styles.resultsCard} style={{ margin: '40px auto', maxWidth: 600, padding: 40, textAlign: 'center', backgroundColor: 'var(--glass-bg)', borderRadius: 24, border: '1px solid var(--border)', boxShadow: '0 8px 32px rgba(0,0,0,0.05)' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', marginBottom: 32, lineHeight: 1.6 }}>
            {percentage >= 80
              ? 'Khả năng nắm bắt đại ý và chi tiết trong độc thoại thương mại của bạn rất ấn tượng!'
              : 'Hãy chú ý các từ chuyển ý và ngữ điệu người nói để xác định mục đích bài nói nhanh hơn!'}
          </p>
