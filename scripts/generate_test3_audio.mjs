...
const AUDIO_DIR = path.join(ROOT_DIR, 'public', 'audio', 'ets2022', 'test3');
const TEMP_DIR = path.join('/tmp', 'toeic_audio_gen_t3');
...
const p1Path = path.join(ROOT_DIR, 'public', 'data', 'ets2022', 'test3', 'part1.json');
...
p1Data.forEach((q, idx) => {
  ...
  q.audioUrl = `/audio/ets2022/test3/${mp3Name}`;
...
const p2Path = path.join(ROOT_DIR, 'public', 'data', 'ets2022', 'test3', 'part2.json');
...
const p3Path = path.join(ROOT_DIR, 'public', 'data', 'ets2022', 'test3', 'part3.json');
...
p3Data.forEach((set, idx) => {
  ...
  set.audioUrl = `/audio/ets2022/test3/${mp3Name}`;
...
const p4Path = path.join(ROOT_DIR, 'public', 'data', 'ets2022', 'test3', 'part4.json');
const p4Data = JSON.parse(fs.readFileSync(p4Path, 'utf8'));

const p4SpeakerVoices = [
  'Samantha', // Set 1: Dental clinic
  'Daniel',   // Set 2: Airport announcement
  'Alex',     // Set 3: Advertisement
  'Samantha', // Set 4: Workplace climate controls
  'Daniel',   // Set 5: Maritime museum tour
  'Alex',     // Set 6: Traffic report
  'Karen',    // Set 7: Dr. Wright introduction
  'Daniel',   // Set 8: Financial performance review
  'Samantha', // Set 9: Store directory
  'Alex',     // Set 10: Conference itinerary
];
...
// Synchronize scripts/test3_data files as well
console.log('\n--- Synchronizing scripts/test3_data files ---');
const t3P1Ts = `export const part1Data = ${JSON.stringify(p1Data, null, 2)};\n`;
fs.writeFileSync(path.join(ROOT_DIR, 'scripts', 'test3_data', 'part1.ts'), t3P1Ts, 'utf8');

const t3P2Ts = `export const part2Data = ${JSON.stringify(p2Data, null, 2)};\n`;
fs.writeFileSync(path.join(ROOT_DIR, 'scripts', 'test3_data', 'part2.ts'), t3P2Ts, 'utf8');

const t3P3Ts = `export const part3Data = ${JSON.stringify(p3Data, null, 2)};\n`;
fs.writeFileSync(path.join(ROOT_DIR, 'scripts', 'test3_data', 'part3.ts'), t3P3Ts, 'utf8');

const t3P4Ts = `export const part4Data = ${JSON.stringify(p4Data, null, 2)};\n`;
fs.writeFileSync(path.join(ROOT_DIR, 'scripts', 'test3_data', 'part4.ts'), t3P4Ts, 'utf8');

console.log('🎉 Successfully generated all 54 local audio files for Test 3!');
